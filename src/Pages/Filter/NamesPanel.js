...
export default function NamesPanel({
  ...
  const namesDatasetoptionalLength = useSelector(
    (state) => state.project.formState.namesDatasetoptionalLength
  );

  React.useEffect(() => {
    if (namesDatasetoptionalLength > 0) {
      setOptionalKeywords(
        [...Array(namesDatasetoptionalLength)].map((u, i) => i + 1)
      );
    } else {
      setOptionalKeywords([]);
    }
  }, [namesDatasetoptionalLength]);
  ...
  const addMoreKeyword = () => {
    ...
    dispatch(change(formName, selectField, "Assignee"));
    ...
    <Paper style={{ marginTop: 15 }}>
      <Box padding={2.2}>
        <Box textAlign="left" display="flex" alignItems="center">
          <div className="Filter_title">{t("NAMES")}</div>
          <Field
            name={OPERATOR_FIELD}
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={6}>
            ...
            >
              ...
              {/* <MenuItem value="Assignee Standardized">
          ...
          <Grid container spacing={3} key={val}>
            ...
            <Grid item xs={12} sm={4}>
              ...
              >
                <MenuItem value="Assignee">Assignee</MenuItem>
                <MenuItem value="Inventor">Inventor</MenuItem>
                {/* <MenuItem value="Assignee Standard">Assignee Standard</MenuItem> */}
              </Field>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Field
