...
export default function MetaDtaPanel({
  classes,
  formName,
  INPUT_CHIP_FIELD,
  FILTERSELECT_FIELD,
  ...
  const metaFirstValue = useSelector((state) => {
    return selector(state, FILTERSELECT_FIELD);
  });

  React.useEffect(() => {
    dispatch(change(formName, FILTERSELECT_FIELD, defaultMetaValue));
    if (metaFirstValue) {
      const arr = [].filter((f) => f.id == metaFirstValue.id);
      if (arr.length) {
        dispatch(change(formName, FILTERSELECT_FIELD, arr[0]));
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [previousMetaData, dispatch, metaFirstValue]);

  const { metaDataDatasetOptionalLength } = useSelector(
    (state) => state.project.formState
  );

  React.useEffect(() => {
    if (metaDataDatasetOptionalLength > 0) {
      setOptionalKeywords(
        [...Array(metaDataDatasetOptionalLength)].map((u, i) => i + 1)
      );
    } else {
      setOptionalKeywords([]);
    }
  }, [metaDataDatasetOptionalLength]);
  ...
  const addMoreKeyword = () => {
    ...
    dispatch(change(formName, selectField, defaultMetaValue));
    ...
    <Paper style={{ marginTop: 15 }}>
      <Box padding={2.2}>
        <Box textAlign="left" display="flex" alignItems="center">
          <div className="Filter_title">{t("METADATA")}</div>
          <Field
            name={OPERATOR_FIELD}
