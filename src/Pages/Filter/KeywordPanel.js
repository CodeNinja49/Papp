...
export default function KeywordPanel({
  ...
  const keywordDatasetOptionalLength = useSelector(
    (state) => state.project.formState.keywordDatasetOptionalLength
  );
  React.useEffect(() => {
    if (keywordDatasetOptionalLength > 0) {
      setOptionalKeywords(
        [...Array(keywordDatasetOptionalLength)].map((u, i) => i + 1)
      );
    } else {
      setOptionalKeywords([]);
    }
  }, [keywordDatasetOptionalLength]);
  ...
  const addMoreKeyword = () => {
    ...
    dispatch(change(formName, selectField, "ALL"));
    ...
    <Paper style={{ marginTop: 15 }}>
      <Box padding={2.2}>
        <Box textAlign="left" display="flex" alignItems="center">
          <div className="Filter_title">{t("KEYWORDS")}</div>
          <Field
            name={OPERATOR_FIELD}
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} sm={6}>
            ...
            >
              ...
              <MenuItem value="Title / Abstract / Claim">
                Title / Abstract / Claim
              </MenuItem>
              <MenuItem value="Title / Abstract">Title / Abstract</MenuItem>
            </Field>
          </Grid>
          <Grid item xs={12} sm={6}>
            <Field
